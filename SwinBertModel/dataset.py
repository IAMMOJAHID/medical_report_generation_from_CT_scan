...
from monai.transforms import Compose, LoadImaged, EnsureChannelFirstd, Orientationd, Spacingd, ScaleIntensityRanged, CropForegroundd, RandFlipd, RandRotate90d, RandShiftIntensityd, RandSpatialCropSamplesd, ToTensord
...
def get_train_transforms():
    return Compose([
        ...
        RandSpatialCropSamplesd(
            ...
            roi_size=(96, 96, 96),  # Size of the cropped region
            num_samples=1,  # Number of crops per image
            random_center=True,  # Randomly select the crop center
...
#     def __init__(self, data_dir, report_dict, transforms):
#         self.image_paths = glob.glob(os.path.join(data_dir, "*.nii.gz"))
...
#         report = self.report_dict.get(file_name, "")
#         sample = self.transforms({"image": image_path})
#         print("sample:", sample)
#         return sample["image"], report
...
def get_dataloaders(train_dir, val_dir, report_file, batch_size=4):
    ...
    train_ds = CacheDataset(data=train_data, transform=train_transforms, cache_num=4, cache_rate=1.0, num_workers=4)
    val_ds = CacheDataset(data=val_data, transform=valid_transforms, cache_num=4, cache_rate=1.0, num_workers=4)
